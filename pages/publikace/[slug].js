...
export default function Home(props) {
  ...
  const { images, title, description } = data.publication;
  const table = data.publication;
...
export const getStaticPaths = async () => {
  const { data } = await client.queries.publicationConnection();
  const paths = data.publicationConnection.edges.map((x) => {
...
export const getStaticProps = async (ctx) => {
  const { data, query, variables } = await client.queries.publication({
